...
class CosseratRodWithCrossSectionalDeformation_OptimizationFunctor;

template <int NumNodes_>
class CosseratRodWithCrossSectionalDeformation : public CosseratRod_Base<NumNodes_, CosseratRodWithCrossSectionalDeformation_State<NumNodes_>>
{
public:
    ...
    using OptimizationFunctor = CosseratRodWithCrossSectionalDeformation_OptimizationFunctor<NumNodes_>;
...
public:
    ...
    CosseratRodWithCrossSectionalDeformation(Real length, const CrossSectionType_& cross_section, Real E, Real nu, bool constrain_base=true, bool constrain_tip=false, bool bending_correction=true)
...
{
public:
    CosseratRodWithCrossSectionalDeformation_OptimizationFunctor(CosseratRodWithCrossSectionalDeformation<NumNodes_>* rod, Vec3r applied_tip_force)
    ...
    Real operator() (const VecXr& x, VecXr& grad)
    {
        ...
        using StateVecType = typename CosseratRodWithCrossSectionalDeformation<NumNodes_>::State::StateVecType;
        StateVecType state = x.head<CosseratRodWithCrossSectionalDeformation<NumNodes_>::State::NumStates>();
        _rod->setState(state);
