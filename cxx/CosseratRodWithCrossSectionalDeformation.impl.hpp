...
template <int N>
Real CosseratRodWithCrossSectionalDeformation<N>::minimizationEnergy(const Vec3r& applied_tip_force) const
{
    ...
    Mat3r M_mat = this->_K.template block<3,3>(0,0);
...
template <int N>
typename CosseratRodWithCrossSectionalDeformation<N>::EnergyGradientType CosseratRodWithCrossSectionalDeformation<N>::minimizationEnergyGradient(const Vec3r& applied_tip_force) const
{
    Real h = this->_length / (N-1);
